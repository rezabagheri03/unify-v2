      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>فرم‌های قابل استفاده در تمام دانشکده‌ها</CardDescription>
          </div>
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button><Plus className="h-4 w-4 ml-1" />فرم سراسری جدید</Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader><DialogTitle>بارگذاری فرم سراسری</DialogTitle></DialogHeader>
              <form onSubmit={(e) => { e.preventDefault(); create.mutate(); }} className="space-y-3">
                ...
                <Button type="submit" disabled={create.isPending || !file}><Upload className="h-4 w-4 ml-1" />بارگذاری</Button>
              </form>
            </DialogContent>
          </Dialog>
        </CardHeader>
        <CardContent className="space-y-2">
          {(forms || []).filter((f: any) => !f.departmentId).map((f: any) => (
            <div key={f.id} className="flex items-center justify-between p-3 border rounded-md">
              ...
              <div className="flex gap-2">
                ...
                <EditFormDialog form={f} onSaved={() => qc.invalidateQueries({ queryKey: ['forms'] })} />
          ...
          {(forms || []).filter((f: any) => !f.departmentId).length === 0 && (
            <p className="text-center text-muted-foreground py-4">فرم سراسری موجود نیست.</p>
          )}
        </CardContent>
      </Card>
    </div>
...
// Golden Doc §3.5.5: Edit form dialog
function EditFormDialog({ form: f, onSaved }: { form: any; onSaved: () => void }) {
  const [open, setOpen] = useState(false);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <form onSubmit={(e) => { e.preventDefault(); update.mutate(); }} className="space-y-3">
          ...
          <div>
            <Label>راهنمای امضا/مهر</Label>
            <Textarea required rows={2} value={form.signatureGuide} onChange={(e) => setForm({ ...form, signatureGuide: e.target.value })} />
          </div>
