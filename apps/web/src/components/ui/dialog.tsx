...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      // a11y: announce to screen readers, trap focus inside
      aria-modal="true"
      // Auto-focus first focusable element when opened (Radix UI default behavior)
      onOpenAutoFocus={(e) => {
        // Let Radix handle focus trap; ensure first input gets focus
        const firstInput = (e.currentTarget as HTMLElement).querySelector<HTMLElement>(
          'input, textarea, select, button:not([data-radix-dialog-close])',
        );
        if (firstInput) {
          e.preventDefault();
          firstInput.focus();
